	...
	public void calculateGoals(String surveySubmissionExternalId) throws DiageoServicesException {
		...
		if(goalsForProcessing.size() > 0) {
			...
			for(JsonNode brand : brands) {
				...
				executeStatement(MessageFormat.format(getSQLStatement("update-flag-on-survey-result-brands"), String.valueOf(brand.get("id").asText()), String.valueOf(calc.processBrands((ObjectNode)brand))));
			}
		}
	}
}
